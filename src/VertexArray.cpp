...
unsigned VertexArrayAttributeBuilder::apply_attribute(unsigned index, unsigned stride, unsigned offset) {
	...
	case GL_FLOAT:
		glVertexAttribPointer(index, attribute.size, GL_FLOAT, false, stride, (void *) offset);
		break;
	case GL_INT:
		glVertexAttribIPointer(index, attribute.size, GL_INT, stride, (void *) offset);
		break;
...
void VertexArrayAttributeBuilder::add_attribute(unsigned size, GLenum gl_data_type, unsigned memory_size) {
	this->attributes.push_back({size,
	                            gl_data_type,
	                            memory_size});
}
